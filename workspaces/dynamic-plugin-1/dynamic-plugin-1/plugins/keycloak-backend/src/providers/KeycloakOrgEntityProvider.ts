...
export class KeycloakOrgEntityProvider implements EntityProvider {
  ...
  static fromConfig(
    ...
    return readProviderConfigs(configRoot).map(providerConfig => {
      console.info(`### debug keycloak plugin: readProviderConfigs function`)
      let taskRunner;
  ...
  async connect(connection: EntityProviderConnection) {
    console.info(`### debug keycloak plugin: connect function`)
  ...
  async read(options?: { logger?: LoggerService }) {
    console.info(`### debug keycloak plugin: read function`)
    if (!this.connection) {
    ...
    console.info(`### debug keycloak plugin: read function - auth`)
    await kcAdminClient.auth(credentials);

    console.info(`### debug keycloak plugin: read function - awaitKeycloakRealm`)
  ...
  schedule(taskRunner: TaskRunner) {
    this.scheduleFn = async () => {
      ...
      console.info(`### debug keycloak plugin: schedule function`)
...
function trackProgress(logger: LoggerService) {
  console.info(`### debug keycloak plugin: trackProgress function`)
